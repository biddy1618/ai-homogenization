...
COLORS = {"high": ["#08519c", "#3182bd", "#6baed6"], "low": ["#e6550d", "#fd8d3c"]}
...
def plot_comparison(series: dict, t_break: int, output: Path) -> None:
    ...
    # stable colour per corpus: cycle within each cognitive-load bucket
    color_of, seen = {}, {"high": 0, "low": 0}
    for folder, (label, cog, d) in series.items():
        color_of[folder] = COLORS[cog][seen[cog] % len(COLORS[cog])]
        seen[cog] += 1
    for ax, metric in zip(axes, METRICS):
        for folder, (label, cog, d) in series.items():
            pre_mean = d[d["_t"] < t_break][metric].mean()
            ax.plot(d["_t"], d[metric] - pre_mean, marker="o", ms=3, lw=1.6,
                    color=color_of[folder],
                    linestyle="-" if cog == "high" else "--",
                    label=f"{label} ({cog}-cog)")
    ...
    fig.suptitle("High- vs low-cognitive-load corpora: change in answer similarity around ChatGPT\n"
                 "(each series centered on its own pre-ChatGPT mean)", fontsize=13)
